<template id="user-calorie">
  <app-layout>
  <div class="uploader container mt-5">
    <h1 class="mb-4 text-center">Upload calorie tracking CSV</h1>
...
<script>
app.component("user-calorie", {
  template: "#user-calorie",
  data() {
    return {
  ...
  methods: {
      ...
      reader.onload = () => {
        try {
          ...
          if (
              ...
              !headers.includes("height") ||
              !headers.includes("velocity") ||
              !headers.includes("userId")
          ) {
            throw new Error(
                "Invalid CSV format. Required headers: weight, height, velocity and userId."
            );
          }
    ...
    async uploadData() {
      const endpoint = "http://localhost:7001/api/calorie";
        ...
        const responses = await Promise.all(
            this.csvData.map((entry) =>
                axios.post(endpoint, {
                  ...
                  velocity: parseFloat(entry.velocity),
