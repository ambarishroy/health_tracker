<template id="user-bloodpressure">
  <app-layout>
  <div class="uploader container mt-5">
    <h1 class="mb-4 text-center">Upload BP CSV</h1>
...
<script>
app.component("user-bloodpressure", {
  template: "#user-bloodpressure",
  data() {
    return {
  ...
  methods: {
    handleFileUpload() {
      ...
      reader.onload = () => {
        try {
          ...
          if (
              !headers.includes("lowerval") ||
              !headers.includes("upperval") ||
              !headers.includes("userId")
          ) {
            throw new Error(
                "Invalid CSV format. Required headers: lowerval, upperval, userId."
            );
          }
    ...
    async uploadData() {
      const endpoint = "http://localhost:7001/api/bloodpressure";

      try {
        const responses = await Promise.all(
            this.csvData.map((entry) =>
                axios.post(endpoint, {
                  lowerval: parseFloat(entry.lowerval),
                  upperval: parseFloat(entry.upperval),
